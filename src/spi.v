...
module spi_master #(
    ...
    // ── CPOL idle level — safe scalar usage ───────────────────
    // CPOL is integer parameter (0 or 1), not a vector.
    // Never use CPOL[0] — bit-indexing a scalar is tool-dependent.
    // Use a wire derived from the parameter instead.
    localparam SCLK_IDLE = (CPOL == 0) ? 1'b0 : 1'b1;
    ...
    // ── Clock divider — uses SCLK_IDLE, not CPOL[0] ──────────
    always @(posedge clk) begin
        if (reset) begin
            clk_div <= 0;
            sclk    <= SCLK_IDLE;
    ...
    // ── CPHA sample/shift edge selection ─────────────────────
    // CPHA=0: sample on rising, shift on falling
    // CPHA=1: sample on falling, shift on rising
