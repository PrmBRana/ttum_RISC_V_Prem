...
module pipeline (
    ...
    // =========================================================
    // RESET SYNCHRONISER + LOCAL REPLICAS (fanout reduction)
    // =========================================================
    ...
    wire reset_sync_orig = reset_ff2;

    // Create 4 local replicas to distribute fanout
    // Each drives ~1/4 of the design
    reg reset_sync_c0, reset_sync_c1, reset_sync_c2, reset_sync_c3;
    always @(posedge clk) begin
        if (reset) begin
            reset_sync_c0 <= 1'b1;
            reset_sync_c1 <= 1'b1;
            ...
            reset_sync_c0 <= reset_sync_orig;
            reset_sync_c1 <= reset_sync_orig;
            reset_sync_c2 <= reset_sync_orig;
            reset_sync_c3 <= reset_sync_orig;
        end
    end

    // Assign each local reset to a different section
    wire reset_sync_fetch = reset_sync_c0;  // PC, IMEM, UART
    wire reset_sync_decode = reset_sync_c1; // IF_ID, Control, RegFile
    wire reset_sync_execute = reset_sync_c2; // EX_stage, ALU, MEM_stage
    wire reset_sync_writeback = reset_sync_c3; // WriteBack_stage
    wire reset_sync_periph = reset_sync_c0; // Bootloader, DataMem, SPI, GPIO
    ...
    // =========================================================
    // FETCH STAGE (using reset_sync_fetch)
    // =========================================================
    ...
    pc_register Register_top (
        .clk(clk), .reset(reset_sync_fetch),
    ...
    // =========================================================
    // DECODE STAGE (using reset_sync_decode)
    // =========================================================
    IF_ID_stage IF_DF_top (
        .clk(clk), .reset(reset_sync_decode),
    ...
    // =========================================================
    // EXECUTE STAGE (using reset_sync_execute)
    // =========================================================
    EX_stage ex_stage (
        .clk(clk), .reset(reset_sync_execute), .flushE(FlushE_top),
    ...
    MEM_stage mem_stage (
        .clk(clk), .reset(reset_sync_execute),
    ...
    // =========================================================
    // WRITEBACK STAGE (using reset_sync_writeback)
    // =========================================================
    WriteBack_stage writeback_stage (
        .clk(clk), .reset(reset_sync_writeback),
